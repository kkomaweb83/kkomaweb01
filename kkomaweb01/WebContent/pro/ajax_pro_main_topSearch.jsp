...
<table><tr>
	...
	<img src="<%=cp %>/img/close_btn.gif" border="0"></a>
	</span></td>
	</tr>
</table>
</div>
...
<c:if test="${!empty mkrList}">
	...
	<input type="checkbox" name="mkr_no_sy" onclick="goMainTopChk(this);" 
				value="${bean.mkr_no }" />${bean.mkr_name }
	</div>
	</c:forEach>
</c:if>
</td>
...
<table style="width: 946px;">
<tr><td colspan="3" style="background-color: #ffdab9; text-align: center; height: 23px;">
<span style="color: blue;">${bean.pcl_name }</span></td></tr>
<c:forEach var="bean2" items="${bean.pcl_list}" varStatus="no2">
<tr><td style="width: 120px; padding-left: 3px; vertical-align: middle; border-bottom: gray 1px dotted;">
...
<div style="float: left; width: 160px;">
<input type="checkbox" name="pdt_step51_sy" onclick="goMainTopChk(this);"  
			value="${bean3.pcl_no }" />${bean3.pcl_name }
</div>		
</c:forEach>
</td>
</tr>
