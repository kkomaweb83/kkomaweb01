...
<tr>
	...
	<img src="<%=cp %>/product_img/${proVo.pmg_file }" style="width: 300px; height: 300px; border: 0px;">
	</td>
	<td width="550" valign="middle">
		...
		<img src="<%=cp %>/img/btn_buy.gif" border="0">
		</c:if>
		<c:if test="${login.cmd == 101}">
		<a href="<%=cp %>/ProController?dana=sct_odr_doc&sct_pro_no=${proVo.pro_no }
																&sct_pro_part=1
																&sct_pro_muti=2
		...
		<c:if test="${login.cmd != 101}">
		<img src="<%=cp %>/img/btn_cart.gif" border="0">
		</c:if>
		<c:if test="${login.cmd == 101}">
